...
MODEL_MODE = os.getenv("MODEL_MODE", "adk_agent_engine") # Defaulting to new mode
...
def bot_chat():
    ...
    try:
        ...
        request_data = request.get_json() # Use get_json() which handles parsing
        if not request_data:
        ...
        if router_llm and router_prompt:
            try:
                ...
                decision = 'UNKNOWN' # Fallback if router fails, allowing generic response
        ...
        # Try to capture more context from the request if it exists
        try:
